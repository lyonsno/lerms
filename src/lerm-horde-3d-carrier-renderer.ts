...
export async function createExactCarrierLiveSource(): Promise<ExactCarrierLiveSource> {
  return loadExactCarrierLiveSource(typeof document !== 'undefined');
}
  ...
  if (
    ...
    !(
      (actorFrame.lifecycle.phase === 'traversing' &&
        ...
        pose !== null &&
        Number.isFinite(pose.sourceDistance) &&
        Number.isFinite(pose.squirm.phase) &&
        pose.squirm.phaseSource === 'route-distance-v0' &&
        pose.squirm.terrainSupportProfile.length === 7 &&
        pose.squirm.terrainSupportProfile.every(
          (sample, index) =>
            Number.isFinite(sample.t) &&
            Number.isFinite(sample.localOffset) &&
            Math.abs(sample.t - index / 6) < 1e-6,
        ) &&
