...
assert.match(
  liveHandSource,
  /if\s*\(\s*!fluidAssayMode\s*\)\s*publishFluidPacketForFrame\(frame\)/,
...
assert.match(
  liveHandSource,
  /economics:\s*liveFluidEconomics/,
  'the assay packet must carry the normalized fluid economics contract',
);
assert.match(
  liveHandSource,
  /source_flux_particles_per_second:\s*liveFluidEconomics\.sourceFluxParticlesPerSecond\s*\/\s*emitterCount/,
  'the assay must divide requested source flux across active emitters without multiplying it',
);
assert.match(
  liveHandSource,
  /requestedActiveParticleBudget[\s\S]*sourceFluxParticlesPerSecond[\s\S]*opticalDensityScale[\s\S]*reconstructionRadiusScale[\s\S]*lifetimeSeconds/,
  'the assay hook must accept the complete composed fluid-economics envelope',
);
assert.match(
  liveHandSource,
...
assert.match(
  witnessSource,
  /--active-budget[\s\S]*--source-flux[\s\S]*--optical-density[\s\S]*--reconstruction-radius[\s\S]*--lifetime/,
  'the witness must expose every composed fluid-economics control',
);
assert.match(
  witnessSource,
