...
assert.equal(packet.workbenchBinding.schema, 'lerms.glove-well-kaminos-workbench-binding.v0');
assert.equal(packet.workbenchBinding.kaminos.cartridgeId, 'lerms-terrarium');
assert.equal(packet.workbenchBinding.kaminos.chamberId, 'lerms-underhill');
...
assert.equal(packet.workbenchBinding.kaminos.outputClass, 'gap_report');
assert.match(packet.workbenchBinding.kaminos.operatorRouteQuery, /kaminos_forge_host=live/);
assert.match(packet.workbenchBinding.kaminos.operatorRouteQuery, /world_cartridge=lerms-terrarium/);
assert.match(packet.workbenchBinding.kaminos.operatorRouteQuery, /world_crucible=glove-emitter/);
assert.match(packet.workbenchBinding.kaminos.operatorRouteQuery, /forge_host_smoke_offer=glove-emitter-native-host-smoke-offer/);
assert.equal(packet.workbenchBinding.firstUse.armature, 'native-host');
assert.equal(packet.workbenchBinding.firstUse.handle, 'source-route');
assert.equal(packet.workbenchBinding.firstUse.firing, 'glove-well-native-host-source-packet');
assert.equal(packet.workbenchBinding.receipt.expectedSchema, 'kaminos.forge-host.smoke-receipt.v0');
assert.equal(packet.workbenchBinding.receipt.currentOutputClass, 'gap_report');
assert.ok(packet.workbenchBinding.downgrades.includes('kaminos_offer_authority_gap_report_route'));
